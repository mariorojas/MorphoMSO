 ...
 */
public class SendFace extends AsyncTask<Void,Void,Void> {
    ...
    public SendFace(String id, byte[] foto, Context context, ConsumeResponse consumeResponse){
        this.id = id;
    ...
    private void sendItem(String dataB){
        ...
        if(dataB != null) {
            ...
            try {
                clientWS.postCon("/api/customer/setPhoto", dataB,Util.getSettingWSPort(context));
            } catch (Exception e) {
                e.printStackTrace();
