 ...
 */
public class SendFaceAut extends AsyncTask<Void,Void,Void> {
    ...
    public SendFaceAut(String id, byte[] foto, Context context, ConsumeResponse consumeResponse){
        this.id = id;
    ...
    private void sendItem(String dataB){
        ...
        if(dataB != null) {
            ...
            try {
                clientWS.postCon("/api/customer/setPhotoAut", dataB,Util.getSettingWSPort(context));
            } catch (Exception e) {
                e.printStackTrace();
