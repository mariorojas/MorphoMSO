 ...
 */
public class SendFingersIden extends AsyncTask<Void,Void,Void> implements ConsumeResponse {
    ...
    public SendFingersIden(Context context, ConsumeResponse consumeResponse){
    ...
    private void sendItem(String dataB,String key,boolean fin,boolean bo,boolean first){
        ...
        if(dataB != null) {
            ...
            try {
                clientWS.postCon("/api/customer/setFingersIden", dat.toString(),Util.getSettingWSPort(context));
            } catch (Exception e) {
                e.printStackTrace();
                allGood = false;
                if (consumeResponse != null)
