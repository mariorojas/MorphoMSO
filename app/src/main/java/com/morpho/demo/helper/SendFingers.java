 ...
 */
public class SendFingers extends AsyncTask<Void,Void,Void> implements ConsumeResponse {
    ...
    public SendFingers(Context context, ConsumeResponse consumeResponse){
    ...
    private void sendItem(String dataB,String key,boolean fin,boolean bo,boolean first){
        if(dataB != null) {
            ...
            try {
                clientWS.postCon("/api/customer/setFingers", dat.toString(),Util.getSettingWSPort(context));
                clientWS.toString();
